...
/*
 * The Interlocked*64 primitives are unavailable on 32-bit x86 targets old
 * enough not to expose them, so supply them here with a cmpxchg8b loop.  Doing
 * so lets the rest of this file take the OSSL_USE_INTERLOCKEDOR64 path
 * unconditionally.
 */
 ...
 * below is guaranteed to be ONCE_UNINITED.  The Interlocked*
...
int CRYPTO_THREAD_cleanup_local(CRYPTO_THREAD_LOCAL *key)
{
    ...
    pdtor = &ossl_win_tls_dtors;
    for (curr = *pdtor; curr != NULL; curr = curr->next) {
        if (curr->key == *key) {
