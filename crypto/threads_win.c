...
int CRYPTO_THREAD_run_once(CRYPTO_ONCE *once, void (*init)(void))
{
    ...
    for (;;) {
        switch (InterlockedCompareExchange(state, OSSL_ONCE_RUNNING,
                                           OSSL_ONCE_UNINITED)) {
        case OSSL_ONCE_DONE:
            /* initialisation already completed */
            return 1;
        case OSSL_ONCE_UNINITED:
            /* we won the race: run the initialiser exactly once */
            init();
            InterlockedExchange(state, OSSL_ONCE_DONE);
            return 1;
        default:
            /* another thread is running the initialiser; wait for it */
            Sleep(1);
            break;
        }
    }
